...
public class Dictionary {
	...
	*/
	public int pageSearch(String target){
		return this.dictionary.indexOf(target);
	}
	...
	*/
	public int eliminateDuplicates(String target){
		int count = 0;
		 ArrayList<String> dic = new ArrayList<>(this.dictionary.size());
		 dic = dictionary;
	...
	*/
	public ArrayList<String> reverseDictionary(){
		if(this.dictionary.size() > 0)
			{
		    ArrayList<String> rDic = new ArrayList<>(this.dictionary.size());
		    
		    for(int i = (dictionary.size() - 1); i <= 0; i--)
		    {
		    	rDic.add(dictionary.get(i));
		    }
